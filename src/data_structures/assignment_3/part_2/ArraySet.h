...
public:
   ArraySet();
   ArraySet(const ItemType initialArray[], const int& count);
...
template<class ItemType>
ArraySet<ItemType>::ArraySet(const ItemType initialArray[], const int& count)
{
   itemCount = 0;
   maxItems = count;
...
bool ArraySet<ItemType>::add(const ItemType& newEntry)
{
   ...
   bool duplicateItem = contains(newEntry);
	if (hasRoomToAdd && !duplicateItem)
	{
		items[itemCount] = newEntry;
		itemCount++;
	}  // end if
    
	return hasRoomToAdd && !duplicateItem;
}  // end add
